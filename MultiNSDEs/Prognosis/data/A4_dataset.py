...
from Common_Functions.data.helpers_datasets import (
    ...
    scale_data, log_scaler, assign_a4_dose_protocol)
...
class A4Dataset(Dataset):
    def __init__(self, config, data):
        ...
        if config.Val_Scenario in [2, 4]: # placebo simulation
            de_rhs_df["AMT"] = 0
        elif config.Val_Scenario in [3, 5]: # TRT simulation
            de_rhs_df["AMT"] = de_rhs_df["TIME"].apply(assign_a4_dose_protocol)
        ...
        bids = de_rhs_df['BID'].unique()
        all_times_doses = pd.DataFrame([(bid, time) for bid in bids for time in union_set], columns=['BID', 'TIME'])
        amt_df = all_times_doses.merge(de_rhs_df[['BID', 'TIME', 'AMT']], on=['BID', 'TIME'], how='left')
        amt_df = amt_df.sort_values(['BID', 'TIME']).reset_index(drop=True)
        amt_df['AMT'] = amt_df.groupby('BID')['AMT'].ffill()

        sorted_times = sorted(union_set)
        amt_df['TIME'] = pd.Categorical(amt_df['TIME'], categories=sorted_times, ordered=True)
        amt_df = amt_df.pivot(index='BID', columns='TIME', values='AMT')
        amt_df = amt_df.sort_index()
        self.rhs_data = torch.tensor(amt_df.values).unsqueeze(-1)
        ...
        # config.s_data_dim is the variable that needs to be changed
