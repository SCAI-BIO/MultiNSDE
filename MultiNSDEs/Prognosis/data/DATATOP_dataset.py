...
class DATATOP_Dataset(Dataset):
    def __init__(self, config, data):
        ...
        # 0 is PBO and [1,2,3] is TRT
        placebo_flag = de_rhs_df[de_rhs_df.TIME == 0]
        self.placebo_flag = torch.tensor(placebo_flag.TREAT.values).unsqueeze(-1).float()
        self.placebo_flag = (self.placebo_flag == 0).float() #we put 0 for placebo and 1 for the rest
        ...
        self.Tmax = max(T)
        self.T = T / self.Tmax if config.norm_time else T
        self.SUBJID = torch.from_numpy(long_data[0].PDDOCID.values).unique()

        # We have 2 treatments so 3 different treatment cases and 1 pbo
        drug_type = de_rhs_df[de_rhs_df.TIME == 0]
        drug_type = (torch.tensor(drug_type.TREAT.values).unsqueeze(-1).float()).expand(-1, self.T.size(0))
    ...
    def get_XW(self):
        return self.long_data, self.long_miss_mask
