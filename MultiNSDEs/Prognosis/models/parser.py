def base_parser():
    ...
    parser.add_argument('--rev_lenc', type=int,
                        default=1)
    parser.add_argument('--nlayers_rnn_enc', type=str,
                        default='2')
    ...
    parser.add_argument('--type_hivae', type=str,
                        default='SLR_BL_HIVAE',
                        choices=['SLR_BL_HIVAE'])
    ...
    parser.add_argument('--act_de', type=str,
                        default='none',
                        choices=['none', 'relu', 'tanh', 'selu', 'softplus', 'sigmoid'])
    parser.add_argument('--rhs_ldim', type=str,
                        default='50')
...
def adapt_parser(config):
    ...
    # Adding try eception ecause optuna. If an exception occurs 
    # it is because we are not optimizing those values
    # String variables
    try:
        config.type_lenc = config.type_lenc.split(',')
        if len(config.type_lenc) != num_enc:
            config.type_lenc = config.type_lenc * num_enc
    except:
        pass
    try:
    ...
    # Int variables
    try:
        config.nlayers_rnn_enc = [int(i) for i in config.nlayers_rnn_enc.split(',')]
        if len(config.nlayers_rnn_enc) != num_enc:
            config.nlayers_rnn_enc = config.nlayers_rnn_enc * num_enc
    except:
        pass
    try:
