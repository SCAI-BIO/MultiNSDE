#!/usr/bin/ipython
import os
import warnings
import numpy as np
import torch
from parser import base_parser
from train import Train
from validation import Validation
from trainval import TrainVal
import sys
sys.path.append('../../')
from Common_Functions.models.utils import define_logs
from Causal.data.load_data import load_dataset, load_cv_datasets

warnings.filterwarnings('ignore')
torch.autograd.set_detect_anomaly(True)

def main(config):

    print('Mode:', config.mode)
    if config.mode == 'cv':
        config, dataloader, dataloader_val = load_cv_datasets(config)
        TrainVal(config, dataloader, dataloader_val)
    else:
        config, dataloader = load_dataset(config)

        if 'train' in config.mode:
            Train(config, dataloader)
            print('Training has finished')
        else:
            Validation(config, dataloader)
            print('Validation has finished')

    
if __name__ == '__main__':

    config = base_parser()
    if config.GPU != '-1':
        os.environ["CUDA_VISIBLE_DEVICES"] = config.GPU
        config.GPU = [int(i) for i in config.GPU.split(',')]
    else:
        config.GPU = False

    if 'A4_Causal' == config.dataset:
        config.longdata_fname = [i for i in config.longdata_fname.split(',')]
        config.longtypes_fname = [i for i in config.longtypes_fname.split(',')]
        if config.num_lenc != len(config.longdata_fname):
            print("Caution: The number of file names provided is less than the number of encoders specified." \
                  " Adjusting the number of encoders to match the available file names.")
            config.num_lenc = len(config.longdata_fname)

    assert config.nlayers_projec > 0, ( 
        'To use this model you need to have at least 1 layer for the projection encoder')

    np.random.seed(config.seed)
    torch.manual_seed(config.seed)
    if torch.cuda.is_available():
        torch.cuda.manual_seed(config.seed)

    config.train_dir = os.path.join(config.train_dir, config.dataset)
    config.or_model_path = os.path.join(config.save_path, config.dataset, 'OR_ATEmodels')

    if config.mode == 'cv':
        config.save_path = os.path.join(
            config.save_path, config.dataset, 
            config.exp_name, 'lambda_OR_TRT_%.5f'%(config.lambda_OR_TRT))
    else:
        config.save_path = os.path.join(
            config.save_path, config.dataset, 
            config.exp_name, 'Fold%d'%(config.train_fold))

    config.save_path_samples = os.path.join(config.save_path, 'samples')
    config.save_path_models = os.path.join(config.save_path, 'models')
    config.save_path_losses = os.path.join(config.save_path, 'losses')

    os.makedirs(config.save_path, exist_ok=True)
    os.makedirs(config.save_path_samples, exist_ok=True)
    os.makedirs(config.save_path_models, exist_ok=True)
    os.makedirs(config.save_path_losses, exist_ok=True)

    config.save_path_losses = os.path.join(config.save_path_losses, 'losses.txt')

    # Print the parser options of the current experiment
    # in a txt file saved in repo/models/exp_name/log.txt
    if 'train' in config.mode:
        define_logs(config)

    main(config)
