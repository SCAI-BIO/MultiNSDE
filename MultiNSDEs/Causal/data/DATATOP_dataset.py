...
class DATATOP_Dataset(Dataset):
    ...
    @staticmethod
    def _extract_arm_columns(df, base_name, n_arms):
        """Return {arm_idx: col_name} from flexible arm-specific naming patterns."""
        arm_to_col = {}
        for col in df.columns:
            low = col.lower()
            if base_name not in low:
                continue

            match = re.search(r'(?:arm|treat|trt)?[_\- ]?(\d+)', low)
            if match is None:
                continue

            arm_idx = int(match.group(1))
            if 0 <= arm_idx < n_arms:
                arm_to_col[arm_idx] = col
    ...
    def __init__(self, config, data):
        ...
        if config.mode == 'cv':
            if static_data is not None:
                config.mode = 'train' if static_data.TRAIN.iloc[0] == 1 else 'val'
            else:
                config.mode = 'train' if scores.TRAIN.iloc[0] == 1 else 'val'
        ...
        if config.type_hivae in ['IC_BL_Causal_HIVAE', 'SLR_BL_Causal_HIVAE']:
            ...
            bl_data, self.scaling_bl_stats = scale_data(
                config, [bl_data], [bl_names], [bl_types], path_=path_bl_, HIVAE_IL=True
            )
            bl_data, bl_miss_mask, bl_types = process_static(bl_data[0], bl_names, bl_types)
        ...
        doses_times = set(de_rhs_df['TIME'].unique().tolist())
        union_set = union_set.union(doses_times)

        base_cols = ['PDDOCID', 'TIME']
        leakage_pattern = re.compile(r'^(?:Fold\d+|CV\d+|LAST_VISIT_FLAG)$', re.IGNORECASE)
        cols = [c for c in de_rhs_df.columns if c not in ['TRAIN', 'TREAT']]
        cols_ffil = [
            c for c in cols
            if c not in ['PDDOCID', 'TIME'] and leakage_pattern.match(c) is None
        ]

        all_times_doses = pd.DataFrame(
            [(pid, time) for pid in unique_ptnos for time in union_set], columns=base_cols
        )
        ...
        arm_rhs_templates = np.zeros((4, len(sorted_times), base_rhs_feat))
        ...
        for d, col in enumerate(cols_ffil):
            ...
            for arm in range(4):
                arm_slice = de_rhs_df[pd.to_numeric(de_rhs_df['TREAT'], errors='coerce') == arm][['PDDOCID', 'TIME', col]].copy()
                if arm_slice.empty:
                    continue

                arm_slice['TIME'] = pd.Categorical(arm_slice['TIME'], categories=sorted_times, ordered=True)
                arm_pivot = arm_slice.pivot(index='PDDOCID', columns='TIME', values=col).sort_index(axis=1)
                arm_pivot = arm_pivot.reindex(columns=sorted_times)
                arm_rhs_templates[arm, :, d] = arm_pivot.mean(axis=0, skipna=True).fillna(0.0).values
        ...
        self.rhs_feature_names = cols_ffil
        self.treat_block_start = treat_start
        self.arm_rhs_templates = torch.tensor(arm_rhs_templates).float()

        if config.Val_Scenario in [2, 3, 4, 5]:
            forced_treat = {2: 0, 3: 1, 4: 2, 5: 3}[config.Val_Scenario]
            if treat_start > 0:
                arr[:, :, :treat_start] = arm_rhs_templates[forced_treat]
        ...
        for i in range(len(long_Ts)):
            name = 'TIME_%d' % (i)
            df_de_miss[name] = df_de_miss['DETIME'].apply(
                lambda x: 1 if x in long_Ts[i].numpy() else 0
            )

            de_miss_i = torch.from_numpy(
                df_de_miss[name].repeat(len(long_names[i])).values.reshape(-1, len(long_names[i]))
            )
        ...
        score_df = scores.drop_duplicates(subset=['PDDOCID']).set_index('PDDOCID').reindex(unique_ptnos)
        if score_df.index.isna().any():
            raise ValueError('Missing score rows for one or more DATATOP patients.')

        pi_col_map = self._extract_arm_columns(score_df, 'propensity', n_treat)
        obs_col_map = self._extract_arm_columns(score_df, 'observation', n_treat)

        # Backward compatibility with legacy binary files.
        has_scalar_pi = 'Propensity_Score' in score_df.columns
        has_scalar_obs = 'Observation_Score' in score_df.columns

        if len(pi_col_map) == n_treat:
            pi_mat = np.stack([
                pd.to_numeric(score_df[pi_col_map[k]], errors='coerce').values
                for k in range(n_treat)
            ], axis=1)
            pi_vals = pi_mat[np.arange(len(unique_ptnos)), baseline_treat.values]
        elif has_scalar_pi:
            pi_vals = pd.to_numeric(score_df['Propensity_Score'], errors='coerce').values
        else:
            raise ValueError(
                'Could not find propensity score columns. Expected either "Propensity_Score" '
                'or one column per arm containing "Propensity" and an arm index (0..3).'
            )

        if len(obs_col_map) == n_treat:
            obs_mat = np.stack([
                pd.to_numeric(score_df[obs_col_map[k]], errors='coerce').values
                for k in range(n_treat)
            ], axis=1)
            obs_vals = obs_mat[np.arange(len(unique_ptnos)), baseline_treat.values]
        elif has_scalar_obs:
            obs_vals = pd.to_numeric(score_df['Observation_Score'], errors='coerce').values
        else:
            raise ValueError(
                'Could not find observation score columns. Expected either "Observation_Score" '
                'or one column per arm containing "Observation" and an arm index (0..3).'
            )

        if np.isnan(pi_vals).any() or np.isnan(obs_vals).any():
            raise ValueError('Missing propensity/observation scores for one or more DATATOP patients.')

        # Avoid unstable inverse weighting when upstream models produce extreme probabilities.
        pi_vals = np.clip(pi_vals.astype(float), 1e-3, 1 - 1e-3)
        obs_vals = np.clip(obs_vals.astype(float), 1e-3, 1 - 1e-3)

        self.Pi_Score = torch.from_numpy(pi_vals).float()
        self.Obs_Score = torch.from_numpy(obs_vals).float()

        self.SUBJID = torch.tensor(unique_ptnos)
        self.drug_type = (
            torch.tensor(baseline_treat.values).unsqueeze(-1).float().expand(-1, self.T.size(0))
        )
        ...
        if config.time_to_event:
            ...
            self.Time_TE = torch.tensor(time_wide.values).float()
            self.Event_TE = torch.tensor(event_wide.values).float()
        else:
            self.Time_TE = torch.ones(len(unique_ptnos), 1).float() * -1
            self.Event_TE = torch.ones(len(unique_ptnos), 1).float() * -1
