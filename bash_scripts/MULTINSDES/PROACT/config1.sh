...
# Dataset
dataset_=PROACT
longdata_fname='PROACT_Long_WithoutDeath_Month_Fold1.csv'
longtypes_fname='PROACT_Long_WithoutDeath_Types.csv'
staticdata_fname='PROACT_Static_Fold1.csv'
statictypes_fname='PROACT_Static_Types.csv'
dosesdata_fname='PROACT_RHS_Month_Fold1.csv'
timetoevent_fname='PROACT_TimeToEvent.csv'
...
s_freq=10 # model weights will be saved every s_freq epochs
...
# Loss 
l_recstat=5.0 # lambda for the static HIVAE loss 
l_reclong=15.0 # lambda for the longitudinal reconstruction loss
l_kllong=50.0 # lambda for the longitudinal KL (beta-VAE)
l_ic=0.001 # lambda for the IC loss
l_te=5.0 # lambda for Time to Event loss

# Longitudinal Encoder
num_enc=2 # If 2, img feats will be used, if 3 lab test data also is used
...
nhidden_lenc=64 # Hidden state of the Longitudinal encoder
...
long_ldim=70 # Longitudinal latent dimension (Z0_long)
long_impl=1 # Defines if there is missing values in the long data
comb_longloss=avg

# Longitudinal Decoder
type_ldec=DIST # Type of encoder
nl_mlp_dec=3 #  If type_ldec == DIST, then it is th number of layers for "y layer", otherwise number of layers of the MLP longitudinal decoder
...
mse_head=0
clipping=1 # Define if the model uses clipping for the MSE heads
inv_ic_loss=1 # Wassertein regularization

# HI-VAE
type_hivae=SLR_BL_HIVAE # If static_data=0, please use Z0_HIVAE as well
stat_ldim=10 # Static latent dimension (Z0_HIVAE)
s_dim_static=4 # Dimension of the sn of the Gaussian Mixture of the static data
...
norm_nde=none # Normalization of the Neural SDE/ODE

name=MultiNSDE_OfficialParams